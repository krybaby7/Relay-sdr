...
export default function App() {
  ...
  return <div className="shell">
    <aside className="sidebar"><a className="brand" href="/#overview"><span className="brand-symbol">r</span><span>relay <small>SDR</small></span></a><span className="nav-caption">WORKSPACE</span><nav aria-label="Main navigation">{[['grid','Overview','/#overview'],['people','Leads','/leads'],['phone','Lab','/#lab'],['clock','Calls','/#calls'],['link','Connections','/#connections']].map(([icon,name,href]) => <a key={name} href={href} className={name === 'Leads' ? 'nav-item active' : 'nav-item'} title={name}><Icon name={icon}/><span>{name}</span>{name === 'Leads' && <span className="nav-dot"/>}</a>)}</nav><details className="mobile-navigation"><summary aria-label="Open navigation">Menu</summary><nav aria-label="Mobile navigation">{[["Overview","/#overview"],["Leads","/leads"],["Lab","/#lab"],["Calls","/#calls"],["Connections","/#connections"]].map(([name,href]) => <a key={name} href={href}>{name}</a>)}<button onClick={lockWorkspace}>Lock workspace</button></nav></details><div className="sidebar-bottom"><div className="workspace-avatar">R</div><div><strong>Private workspace</strong><small>Local · single operator</small></div><button className="icon-button" aria-label="Lock workspace" onClick={lockWorkspace}><Icon name="lock" size={16}/></button></div></aside>
