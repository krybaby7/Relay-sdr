...
import { CallViewer, LeadDetail } from './details';
...
export default function App() {
  ...
  const busy = useRef(false); const lastInteraction = useRef(0); const bootRef = useRef(boot); const queryRef = useRef(query); const viewRef = useRef(viewId); const detailRef = useRef(detail);
  bootRef.current = boot; queryRef.current = query; viewRef.current = viewId; detailRef.current = detail;
  busy.current = detailOpen || !!call || !!dialog || !!layoutDraft || dragging || selected.size > 0;
  ...
  const loadBootstrap = useCallback(async (initial = false) => {
    try {
      ...
      setBoot(fresh); setViewId(id); setQuery(fresh.spec.views.find(v => v.id === id)!.query); setPending(false); setRefreshCounter(n => n + 1);
      return fresh;
    } catch (e) {
  ...
  useEffect(() => {
    ...
    const controller = new AbortController(); const timer = setTimeout(() => {
      ...
      ]).then(([records, metrics, tasks, calls, activity]) => { setData({ records, metrics, tasks, calls, activity: activity.items }); setLoading(false); }).catch(e => { if (!controller.signal.aborted) { setError((e as Error).message); setLoading(false); } });
    }, query.search ? 250 : 0);
  ...
  async function openLead(id: string) { setError(''); try { const result = await api<Detail>(`/api/workspace/leads/${id}`); setDetail(result); setDetailOpen(true); } catch (e) { setError((e as Error).message); } }
  async function refreshDetail() { if (detailRef.current) setDetail(await api<Detail>(`/api/workspace/leads/${detailRef.current.lead.id}`)); setRefreshCounter(n => n + 1); }
  ...
  function showEvidence(ref: Reference) {
    if (ref.note_id) { setDetailOpen(true); setNotice(`Supporting human note: ${ref.note_id}. Open Notes & corrections.`); }
  ...
  const context = data ? { bootstrap: boot, data, detail, query, selected, toggleSelection: (id: string) => setSelected(previous => { const next = new Set(previous); if (next.has(id)) next.delete(id); else next.add(id); return next; }),
  ...
  return <div className="shell">
    ...
    <main className="workspace-main"><header className="topbar"><span>Workspace <span className="separator">/</span> <strong>Leads</strong></span><div><span className={`connection-dot ${modelReady ? 'online' : ''}`}/><button className="text-button" onClick={() => setDialog('settings')}>{modelReady ? 'Workspace agent connected' : 'Workspace agent needs setup'}</button><span className="version-tag">v{boot.version}</span></div></header>
      ...
      <div ref={containerRef} className={`workspace-canvas ${loading ? 'refreshing' : ''}`} aria-busy={loading}>{!data || !context ? <div className="skeleton-grid"><div/><div/><div/></div> : <WorkspaceData.Provider value={context}><JSONUIProvider registry={registry} initialState={{}} handlers={{}}>
        {mounted && <Responsive<Breakpoint> width={width} layouts={layouts} breakpoints={{ lg: 1000, md: 700, sm: 0 }} cols={COLS} rowHeight={28} margin={[16,16]} containerPadding={[0,0]} compactor={noCompactor}
        ...
        {detailOpen && detail && <LeadDetail detail={detail} bootstrap={boot} emphasis={view.emphasis} close={() => setDetailOpen(false)} refresh={refreshDetail} openCall={(id, reference) => setCall({ id, reference })} editLead={() => setDialog('edit_lead')} taskChange={taskChange}/>}
    ...
    {call && <CallViewer id={call.id} reference={call.reference} zone={boot.timezone} close={() => setCall(null)}/>}
