...
const titles = {overview:'Overview',leads:'Leads',lab:'Lab',calls:'Call history',connections:'Connections'};
function resolveTab(){
 const raw=location.hash.slice(1);
 if(raw==='playbook'){history.replaceState(null,'',`${location.pathname}${location.search}#lab`);return 'lab';}
...
function render() { if(!data)return; renderNav(); $('#content').innerHTML=({overview,leads,lab,calls,connections}[tab])(); if(tab==='lab')renderTranscript(); }
function overview(){
 ...
 const steps=[['01','Shape your sales playbook','Product facts, qualification, and tone.',done,'lab','Open Lab'],['02','Test the conversation','Use your microphone or a scripted preview.',data.calls.some(c=>c.kind==='browser'),'lab','Open Lab'],['03','Connect your calling provider','Twilio credentials + an approved test number.',twilioReady,'connections','View setup']];
...
function lab(){
 const live=data.lab.live, grok=data.lab.orchestrator, rubric=data.lab.rubric, p=data.playbook;
 const canTest=live.key_configured && live.browser_test_allowed;
 ...
 const weight=(name)=>`<div class="field"><label for="rubric-${name}">${name[0].toUpperCase()+name.slice(1)} weight</label><input id="rubric-${name}" name="${name}" type="number" min="1" max="5" value="${rubric.criteria[name]}" required></div>`;
 return pageHead('Teach both AIs in one place.','Configure GPT-Live and Grok 4.6, give them shared instructions, and test-talk before any real call.',`<span class="chip ${p.approved?'green':'amber'}">${p.approved?'APPROVED PLAYBOOK':'DRAFT PLAYBOOK'}</span>`,'LAB')+
 `<div class="lab-config"><section class="card pad" aria-label="GPT-Live"><div class="row between wrap"><h3>GPT-Live · voice</h3><div class="row wrap"><span class="chip ${live.key_configured?'green':''}">${live.key_configured?'KEY CONFIGURED':'KEY NOT CONFIGURED'}</span><span class="chip ${live.browser_test_allowed?'green':'amber'}">${live.browser_test_allowed?'BROWSER TEST ALLOWED':'BROWSER TEST OFF'}</span></div></div><p>The conversation. Voice, language, and tone are saved with the playbook below. Keys stay on the server.</p><div class="call-meta"><div><span>Live model</span>${esc(live.live_model)}</div><div><span>Backend model</span>${esc(live.backend_model)}</div></div><label class="checkbox"><input type="checkbox" id="lab-browser-test" ${live.browser_test_allowed?'checked':''} ${voice.running?'disabled':''}><span>Allow browser microphone tests</span></label><p class="hint">Off keeps a Live key for telephone only. Scripted preview never needs a key.</p><div class="config-keys"><span>SERVER ENVIRONMENT · RESTART AFTER CHANGES</span><div><code>OPENAI_API_KEY</code> <code>LIVE_MODEL</code> <code>BACKEND_MODEL</code></div></div>${live.key_configured?'':`<div class="info-box amber section-space">Set <code>OPENAI_API_KEY</code> in the server’s <code>.env</code> and restart. Your account must have access to the configured Live and backend models.</div>`}</section>
 <section class="card pad" aria-label="Grok 4.6"><div class="row between wrap"><h3>Grok 4.6 · leads orchestrator</h3><div class="row wrap"><span class="chip ${grok.enabled_by_server?'green':'amber'}">${grok.enabled_by_server?'ENABLED':'DISABLED IN .ENV'}</span><span class="chip ${grok.configured?'green':''}">${grok.configured?'KEY CONFIGURED':'KEY NOT CONFIGURED'}</span><span class="chip ${grok.paused?'amber':''}">${grok.paused?'PAUSED':'RUNNING WHEN CONFIGURED'}</span></div></div><p>Transcripts and layout only. No dialing, booking, export, or calling tools. Prospects stay on <a href="/leads">Leads</a>.</p><div class="call-meta"><div><span>Model</span>${esc(grok.model || 'unset — use grok-4.6')}</div><div><span>Provider</span>xAI · no OpenAI Responses</div></div><div class="row wrap"><button class="btn sm" data-action="pause-workspace" ${grok.enabled_by_server?'':'disabled'}>${grok.paused?'Resume Grok reasoning':'Pause Grok reasoning'}</button><a class="text-btn" href="/leads">Open leads workspace ↗</a></div><p class="hint">Pause stops new analysis. An in-flight request may finish. Manual lead editing never needs this key.</p><div class="config-keys"><span>SERVER ENVIRONMENT · SEPARATE FROM THE VOICE KEY</span><div><code>WORKSPACE_ENABLED</code> <code>WORKSPACE_API_KEY</code> <code>WORKSPACE_MODEL</code></div></div>${grok.configured?'':`<div class="info-box amber section-space">Set <code>WORKSPACE_ENABLED=true</code>, a server-side <code>WORKSPACE_API_KEY</code>, and <code>WORKSPACE_MODEL=grok-4.6</code>, then restart. This never falls back to the GPT-Live key.</div>`}<div class="form-section">SALES RUBRIC</div><form id="rubric-form"><input type="hidden" name="expected_version" value="${rubric.version}"><p class="hint">Version ${rubric.version}. Grok extracts claims; the server computes categories. These are evidence points, not probabilities of winning.</p><div class="form-grid">${weight('need')}${weight('fit')}${weight('intent')}${weight('authority')}<div class="field wide"><label for="rubric-description">Approved sales criteria</label><textarea id="rubric-description" name="description" rows="4" minlength="20" maxlength="2000" required>${esc(rubric.description)}</textarea></div></div><div class="form-footer"><label class="checkbox"><input type="checkbox" name="approved" ${rubric.approved?'checked':''}><span>I approve this rubric for commercial assessment.</span></label><button class="btn primary" type="submit">Save rubric ${icon('check')}</button></div><p class="error" id="rubric-error" role="alert"></p></form></section></div>
 <form id="playbook-form" class="card pad section-space"><div class="form-section">INSTRUCTIONS · BOTH AIS</div><p class="hint" style="margin:-8px 0 18px">Offer, ICP, approved facts, and next-step goal teach GPT-Live what to say and Grok how to judge fit. Saving does not change an in-flight call.</p><div class="form-grid"><div class="field"><label for="pb-company">Company name</label><input id="pb-company" name="company" value="${esc(p.company)}" required maxlength="160"></div><div class="field"><label for="pb-agent">Agent name</label><input id="pb-agent" name="agent_name" value="${esc(p.agent_name)}" required maxlength="60"></div>${field('product','What are you offering?',3,'Explain your product, its real value, and its limits. The agent must not invent missing details.')}${field('customer_profile','Who is a good fit?',2)}${field('qualification','What should the agent find out?',3)}${field('facts','Approved facts, pricing, and boundaries',4,'Only put verified company information here. No pricing claims are approved in the default playbook.')}${field('goal','What is the next step?',2,'This pilot can save a meeting or human-callback request. It cannot book a time or transfer the call.')}</div>
 <div class="form-section">GPT-LIVE · VOICE, LANGUAGE, TONE</div><div class="form-grid"><div class="field"><label for="pb-lang">Default language</label><input id="pb-lang" name="language" value="${esc(p.language)}" required></div><div class="field"><label for="pb-voice">Voice</label><select id="pb-voice" name="voice"><option value="marin" ${p.voice==='marin'?'selected':''}>Marin</option><option value="cedar" ${p.voice==='cedar'?'selected':''}>Cedar</option></select></div>${field('tone','Tone and speaking style',2)}</div>
...
async function enter(){await refresh();$('#login').classList.add('hidden');$('#workspace').classList.remove('hidden');tab=resolveTab();render();}
...
window.addEventListener('hashchange',async()=>{const next=resolveTab();if(tab==='lab'&&next!=='lab'&&voice.running)await stopVoice('Voice test ended when you left the lab.');tab=next;query='';render();});
...
document.addEventListener('change',async e=>{
 if(e.target.id==='csv-file'){const file=e.target.files[0];if(file){if(file.size>220000){toast('CSV must be smaller than 220 KB.');return;}$('#csv-text').value=await file.text();}}
 if(e.target.id==='lab-browser-test'){
  try{const allowed=e.target.checked;await post('/api/lab/browser-test',{allowed});await refresh(true);toast(allowed?'Browser voice tests allowed.':'Browser voice tests turned off.');}
  catch(error){toast(error.message);await refresh(true);}
 }
});
document.addEventListener('submit',async e=>{
 const form=e.target;if(!['playbook-form','lead-form','import-form','rubric-form'].includes(form.id))return;
 e.preventDefault();const submit=$('button[type="submit"]',form);submit.disabled=true;
 const err=$('#'+(form.id==='playbook-form'?'playbook':form.id==='lead-form'?'lead':form.id==='rubric-form'?'rubric':'import')+'-error');if(err)err.textContent='';
 try{
   ...
   if(form.id==='rubric-form'){const values=new FormData(form);await api('/api/workspace/rubric',{method:'PUT',body:JSON.stringify({expected_version:Number(values.get('expected_version')),approved:values.has('approved'),description:values.get('description'),criteria:{need:Number(values.get('need')),fit:Number(values.get('fit')),intent:Number(values.get('intent')),authority:Number(values.get('authority'))}})});await refresh(true);toast('Sales rubric saved. New assessments use this version.');}
...
document.addEventListener('click',async e=>{
 ...
 try{
  ...
  else if(action==='pause-workspace'){const pause=!data.lab.orchestrator.paused;await post('/api/workspace/settings/pause',{paused:pause});await refresh(true);toast(pause?'Grok reasoning paused.':'Grok reasoning resumed when the server key allows it.');}
